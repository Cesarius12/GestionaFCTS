...
//importamos las funciones getPracticasById y updatePracticas del archivo practicasService.js
document.addEventListener("DOMContentLoaded", async () => {
  ...
  //Para que haya un formulario dinamico que se adapte al estado de las practicas
  //hacemos que se oculten las secciones de convenio, relacion y programa, primero recogemos sus elementos
...
//creamos una función ocultarSecciones que oculta las secciones de convenio, relacion y programa
  function ocultarSecciones() {
    seccionConvenio.classList.add("oculto");
    seccionRelacion.classList.add("oculto");
    seccionPrograma.classList.add("oculto");
  }
  //llamamos a la función ocultarSecciones que esta arriba creada y oculta las secciones dependiendo del estado elegido en el formulario
  ...
  if (alumnoId) {
    try {
      //al tener presente todos los campos de la tabla es mas facil manejar los datos añadiendole la condicion de que si no hay datos, se añada un null
...
document
  ...
  .addEventListener("submit", async function (event) {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    const alumnoId = urlParams.get("id");
    //lo mismo para la insercion añadiendo el null si no hay datos
    const alumnoData = {
